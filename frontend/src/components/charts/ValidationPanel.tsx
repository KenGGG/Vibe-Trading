  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        将交易顺序随机打乱 {mc.n_simulations.toLocaleString()} 次，以检验夏普比率是否显著优于随机结果。
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-xl border border-border/60 bg-muted/20 p-3">
        ...
        <Stat label="模拟均值" value={mc.simulated_sharpe_mean.toFixed(2)} sub={`标准差 ${mc.simulated_sharpe_std.toFixed(2)}`} />
      ...
      <div className="space-y-1">
        <div className="flex justify-between text-[10px] text-muted-foreground font-mono">
          ...
          <span>实际值: {mc.actual_sharpe.toFixed(2)}</span>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <Badge value={reliable ? "区间 > 0" : "区间包含 0"} good={reliable} />
      </div>
      <p className="text-xs text-muted-foreground">
        对日收益率重采样 {bs.n_bootstrap.toLocaleString()} 次，以估计 {(bs.confidence * 100).toFixed(0)}% 置信区间。
      </p>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 rounded-xl border border-border/60 bg-muted/20 p-3">
        ...
        <Stat label="中位数夏普" value={bs.median_sharpe.toFixed(2)} />
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="border-b text-left text-muted-foreground">
            ...
            <th className="py-1.5 pr-3 text-right">收益率</th>
